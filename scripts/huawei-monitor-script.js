...
const ENABLE_WORKFLOW_LOG = false; // 默认禁用详细工作流日志（减少通知干扰）
...
async function initProductsList() {
  try {
    ...
    if (cachedList) {
      ...
      $.log(`从缓存加载了 ${productsToMonitor.length} 个商品配置`);
    } else {
      // 首次运行，使用默认值并保存
...
// 获取商品历史状态
function getProductStatus(key) {
  try {
    const savedStatus = $.getdata(key);
...
// 保存商品状态
function saveProductStatus(key, status) {
  try {
    const jsonStatus = JSON.stringify(status);
    $.setdata(jsonStatus, key);
  } catch (e) {
    $.log(`保存状态出错: ${e}`);
  }
}
